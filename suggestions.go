...
// GenerateDomainSuggestionsWithContext generates creative domain name suggestions via the
// OpenAI API. It leverages function calling with structured output to ensure suggestions
// are returned in the correct format. All suggested domains end with .com as enforced by
// the system prompt.
//
// Parameters:
...
func GenerateDomainSuggestionsWithContext(ctx context.Context, apiKey, prompt string, count int, opt SuggestOptions) ([]DomainRecord, error) {
	...
	if opt.Model == "" {
		opt.Model = openAIModel
	}
	if opt.BaseURL == "" {
		opt.BaseURL = openAIBaseURL
	}
	hc := opt.HTTPClient
	if hc == nil {
...
// GenerateDomainSuggestions is a backward-compatible wrapper using default context
// and options.
...
func writeSuggestionsFile(path string, list []DomainRecord) error {
	pruned := pruneSuggestions(list)
	data, err := loadAndMergeSuggestions(path, pruned)
	if err != nil {
		return err
	}
...
// pruneSuggestions keeps only the Domain field for suggestions output.
func pruneSuggestions(list []DomainRecord) []DomainRecord {
...
// loadAndMergeSuggestions loads existing grouped data and merges unverified suggestions.
func loadAndMergeSuggestions(path string, pruned []DomainRecord) (ExtendedGroupedData, error) {
	data := ExtendedGroupedData{Unverified: pruned}
	fi, err := os.Stat(path)
	if err == nil && fi.Size() > 0 {
		if fi.IsDir() {
			return ExtendedGroupedData{}, fmt.Errorf("write suggestions: %s is a directory", path)
		}

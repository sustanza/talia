...
type NetWhoisClient struct {
	...
	Server string
	// Timeout defines the per-request timeout when dialing the WHOIS server.
	// A zero value falls back to the defaultWhoisTimeout.
...
func (c NetWhoisClient) Lookup(domain string) (string, error) {
	return c.lookup(context.Background(), domain)
}

// LookupContext executes the WHOIS query using the provided context. If the context does
// not already include a deadline, the client's Timeout is applied.
func (c NetWhoisClient) LookupContext(ctx context.Context, domain string) (string, error) {
	return c.lookup(ctx, domain)
}

func (c NetWhoisClient) lookup(ctx context.Context, domain string) (string, error) {
	timeout := c.Timeout
	if timeout <= 0 {
	...
	if err != nil {
		if ctx.Err() != nil {
			return "", fmt.Errorf("failed to connect to WHOIS: %w", ctx.Err())
		}
...
func CheckDomainAvailabilityWithClient(domain string, client WhoisClient) (bool, AvailabilityReason, string, error) {
	...
	return interpretWhoisResult(resp, err)
}
...
// ContextWhoisClient describes a WHOIS client that supports context cancellation.
type ContextWhoisClient interface {
	LookupContext(ctx context.Context, domain string) (string, error)
}

// CheckDomainAvailabilityWithClientContext runs a WHOIS lookup using a context-aware client.
func CheckDomainAvailabilityWithClientContext(ctx context.Context, domain string, client ContextWhoisClient) (bool, AvailabilityReason, string, error) {
	resp, err := client.LookupContext(ctx, domain)
	return interpretWhoisResult(resp, err)
}

func interpretWhoisResult(resp string, err error) (bool, AvailabilityReason, string, error) {
	if err != nil {
